...
OBDReader.prototype.autoconnectWifi = function (url, instance) {
    var self = instance;
    self.emit('debug', 'Wifi autoconnect called');
    var hp = url.split(':');
    ...
    OBDSocket.open(hp[0], parseInt(hp[1]),
        function (succ) {
            ...
            //Event connected
            //c ("Emitting connected");
            self.emit('connected');
        },
        function (err) {
            self.connected = false;
            self.onWiFiError(err)
        });
    OBDSocket.onData = function (data) {
        self.onWiFiEvent(data, self)
    };
    OBDSocket.onClose = function (data) {
        self.connected = false;
        self.onWiFiDisconnect(data)
    };
    OBDSocket.onError = function (data) {
        self.connected = false;
        self.onWiFiError(data);
    };

    function sockConvert(dataString) {
    ...
    this.intervalWriter = setInterval(function () {
        if (queue.length > 0 && self.connected) {
            try {
                ...
                self.OBDSocket.write(sockConvert(data));
...
// credit: http://ourcodeworld.com/articles/read/164/how-to-convert-an-uint8array-to-string-in-javascript
...
OBDReader.prototype.onWiFiConnect = function () {
    this.emit("debug", "Connected to Wifi dongle");
}
OBDReader.prototype.onWiFiError = function (data) {
   
    this.emit('error', "Wifi Error: " + JSON.stringify(data));

}

OBDReader.prototype.onWiFiEvent = function (u8data, instance) {
...
OBDReader.prototype.onWiFiDisconnect = function () {
    this.emit ('debug', 'wifi disconnected');

}

OBDReader.prototype.autoconnect = function (type, parameter) {
    this.connType = type;
    if (type == "bluetooth") OBDReader.prototype.autoconnectBluetooth(parameter, this)
    else OBDReader.prototype.autoconnectWifi(parameter, this);
}
